declare module F2 {
  ...
  interface ChartProps {
    ...
    id: string; // 指定对应 canvas 的 id
    el: HTMLElement; // 如果未指定 id 时可以直接传入 canvas 对象
    width?: number;
    height: number;
    padding?:
      | {
  ...
  interface ChartAxisConfig {
    /**
     * 坐标轴线的配置信息，设置 null 时不显示
     */
    line?:
      | Styles.line & {
          top: boolean; // true 展示在最上层图形 false 展示在最下层图形。
        }
      | null;
    labelOffset: number;
